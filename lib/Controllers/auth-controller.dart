    ...
    if (passValidator == null) {
      ...
      server.postRequest(endPoint: APIList.login, body: jsonBody).then((response) {
        ...
        if (response != null && response.statusCode == 200) {
          ...
          if (storage.read('userType') == 'customer') {
            isFsUser = true;
          } else {
            isFsUser = false;
          }
          userService.saveString(key: 'user-id', value: loginData.data!.user?.id!.toString());
          userService.saveString(key: 'email', value: loginData.data!.user!.email);
          userService.saveString(key: 'image', value: loginData.data!.user!.image);
          userService.saveString(key: 'name', value: loginData.data!.user!.name);
          userService.saveString(key: 'phone', value: loginData.data!.user!.phone);
  ...
  signupOnTap(hubID, usertype) async {
    ...
    server.postRequest(endPoint: APIList.register, body: jsonBody).then((response) {
      ...
      if (response != null && response.statusCode == 200) {
        ...
        final jsonResponse = json.decode(response.body);
        print(jsonResponse);
        Get.rawSnackbar(message: jsonResponse['data']['message'].toString(), backgroundColor: Colors.red, snackPosition: SnackPosition.TOP);
